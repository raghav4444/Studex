...
import { BookOpen, Users, MessageSquare, User, Home, Calendar, Briefcase, Bell } from 'lucide-react';
    ...
    <header className="bg-[#161b22] border-b border-gray-800 px-4 py-3">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center space-x-1">
          {tabs.slice(0, 7).map((tab) => { // Include Profile tab (was 6, now 7)
            const Icon = tab.icon;
            return (
              <button
                key={tab.id}
      ...
      <nav className="md:hidden mt-3 flex items-center justify-around border-t border-gray-800 pt-3">
        {tabs.slice(0, 6).map((tab) => { // Include Profile tab for mobile (was 5, now 6)
          const Icon = tab.icon;
          return (
            <button
              key={tab.id}
